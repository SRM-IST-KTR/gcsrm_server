...
exports.sendContact = async (req, res, next) => {
  ...
  try {
    ...
    if (!errors.isEmpty()) {
      Sentry.captureMessage('Contact form validation failed', {
        level: 'warning',
        tags: {
    ...
    Sentry.logger.info('Processing contact form submission', {
      operation: 'sendContact',
      email,
      name
    });
    ...
    try {
      ...
      Sentry.logger.info('Contact form emails sent successfully', {
        operation: 'sendContact',
        email,
      ...
      // Log slow email sending (over 5 seconds)
      if (sendDuration > 5000) {
        Sentry.logger.warn('Slow email sending operation', {
          operation: 'sendContact',
